    ...
    <div className="animate-in fade-in duration-500">
      ...
      <div className="bg-[#1C1F26] rounded-[32px] p-6 border border-white/5 mb-8 shadow-2xl">
        <div className="flex items-center justify-between mb-8 px-2">
          <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="text-[#00db9a] p-1"><ChevronLeft size={24} /></button>
          <h2 className="text-xl font-extrabold text-[#00db9a] tracking-tight">{format(currentMonth, 'MMMM')}</h2>
          <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="text-[#00db9a] p-1"><ChevronRight size={24} /></button>
        </div>
            ...
            return (
              ...
              >
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all
                  ${isSelected ? 'bg-[#00db9a] text-black scale-110 shadow-[0_5px_15px_rgba(0,219,154,0.4)]' : 
                    today ? 'text-[#00db9a] border border-[#00db9a]/30' : 
                    isCurrMonth ? 'text-zinc-400' : 'text-zinc-800'}`}
                >
                  {format(day, 'd')}
                </div>
                {hasIdeas && !isSelected && (
                  <div className="absolute bottom-1 w-1 h-1 rounded-full bg-[#00db9a]" />
                )}
              </button>
            );
          })}
      ...
      <div className="space-y-6 pb-12">
        <div className="flex items-center justify-between px-1">
          ...
          <button className="text-[#00db9a] p-1"><Plus size={20} /></button>
        </div>
